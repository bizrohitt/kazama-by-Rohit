...
///
/// A `FutureProvider`, not a plain one, for a reason that is a real behaviour and
/// not a style choice: a ticket LEAVES `openTicketsProvider` the moment it is paid
/// or voided. If the editor vanished with it, the billing screen would snap back to
/// the due list in the same frame the money was accepted — so the cashier never
/// sees "Paid", the change due, or whether the receipt printed. Hence the fallback
/// read: still in the open list, use it (zero extra queries, and every keystroke
/// rebuild is free); otherwise refresh the by-id row ONE last time, which returns
/// the settled bill, greyed by its own status.
final FutureProvider<OrderTicket?> activeTicketProvider = FutureProvider<OrderTicket?>((ref) async {
  ...
  // Not open any more: paid, voided, or served-and-settled. `loadForReceipt` is a
  // plain read (it exists for exactly this — the printer needs the settled ticket),
  // so the screen can still show and reprint what it just closed.
  return ref.read(orderRepositoryProvider).loadForReceipt(id);
});

/// Hand the counter a clean slate. Called when the UI has already shown the result
/// (billing clears after a *fully* settled bill; the order screen's Hold button
/// clears immediately), never from inside the provider — a selection that clears
/// itself while a print is in flight is how a cashier double-taps "Pay".
void settleActiveTicket(WidgetRef ref) => ref.read(activeTicketIdProvider.notifier).state = null;
